...
public class Watermelons {
	...
	/**
	 * Computes the sum of each distinct pair of entries in the incoming array. A
	 * given pair of entries has its sum computed only once. So if you compute the
	 * sum for entries 2 and 4, and that sum appears in your answer array, then you
	 * do not also compute the sum of entries 4 and 2. Depending on the input, the
	 * same sum value may appear multiple times in the result. For example, if all
	 * of the incoming values are 0, then so are all of the returned values.
	 * 
	...
	public static int[] allPairSums(int[] weights) {
		 ...
		 int newLength = pairCount(weights);

//		int newArrayLength = 0;
//
//		for (int z = 1; z < weights.length; z++) {
//			newArrayLength = newArrayLength + (weights.length - z);
//
//		}
//		
	

		int sum[] = new int[newLength];
//
...
//		
	...
	/**
	 * The method below must COMPUTE and return a solution as described on the web
	 ...
	 * 
	 * @param pairSums is array of watermelon pairwise sums. In other words, each
	 *                 element of pairSums represents the sum of one pair of
